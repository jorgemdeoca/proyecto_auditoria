...
<head>
    <meta charset="UTF-8">
    <title>Auditoría de Pagos</title>
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        body { font-family: DejaVu Sans, sans-serif; font-size: 9px; color: #1f2937; background: #fff; }
        .header { background: #3b82f6; color: #fff; padding: 14px 20px; margin-bottom: 14px; }
        ...
        thead th { padding: 6px 8px; text-align: left; font-weight: bold; text-transform: uppercase; color: #1e40af; border-bottom: 2px solid #93c5fd; letter-spacing: 0.03em; }
...
<body>
    ...
    <div class="meta">
        ...
        <span>Módulo: <strong>Pagos</strong></span>
    </div>
    <table>
        <thead>
            <tr>
        ...
        <tbody>
            ...
            <tr>
                ...
                <td>
                    @php $cls = match($reg->event) { 'created' => 'created', 'updated' => 'updated', 'deleted' => 'deleted', default => 'created' }; @endphp
                ...
                <td>
                    @if($reg->new_values)
                        <div class="pre">{{ substr(json_encode(json_decode($reg->new_values), JSON_UNESCAPED_UNICODE), 0, 120) }}</div>
                    @else — @endif
                </td>
    ...
    <div class="footer">Generado automáticamente por el Sistema de Auditoría</div>
</body>
</html>
